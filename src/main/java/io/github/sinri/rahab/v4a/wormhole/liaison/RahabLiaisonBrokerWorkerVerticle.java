...
import io.github.sinri.rahab.RahabChinaIPFilter;
...
class RahabLiaisonBrokerWorkerVerticle extends KeelVerticle {
    ...
    private final boolean shouldAcceptClientOutsideChina;
    ...
    public RahabLiaisonBrokerWorkerVerticle(NetSocket socket, String requestID, ClientSocketManager clientSocketManager) {
        ...
        this.shouldAcceptClientOutsideChina = checkShouldAcceptClientOutsideChina();
    }

    protected static boolean checkShouldAcceptClientOutsideChina() {
        String allow = Keel.getPropertiesReader().getProperty("liaison.broker.worker.allow");
        if (allow == null || allow.trim().length() == 0) {
            return true;
        } else {
            return !allow.equalsIgnoreCase("china");
        }
    }
    ...
    public void start() throws Exception {
        ...
        KeelLogger socketLogger = Keel.standaloneLogger("ProxyBrokerSocket")
                .setContentPrefix(requestID);
        ...
        socketLogger.info(
                "通讯整备中",
                new JsonObject()
                        .put("role", atomicSocketRole)
                        .put("remote", Objects.requireNonNullElse(socket.remoteAddress(), "null").toString())
        );
                ...
                .handler(buffer -> {
                    if (atomicSocketRole.get().equals(SOCKET_ROLE.INIT)) {
                        ...
                        if (matcherForProxyRegister.find()) {
                            ...
                            if (!this.shouldAcceptClientOutsideChina) {
                                ...
                                if (!isChinaMainlandIP) {
                                    socket.write("GFW!");
                                    socketLogger.error("代理卧底通讯 似乎来自非中国大陆IP， 即将关闭 与客户端 [" + requestID + "] 的 通讯");
                                    undeployMe();
                                    return;
                                }
                            }
                                        ...
                                        .onComplete(voidAsyncResult -> {
                                            if (voidAsyncResult.failed()) {
                                                ...
                                                if (currentFailedCount >= 5) {
                                                    socketLogger.fatal("Broker Pong 连续5次暴毙！<del>撤销PING的定时器并准备重启。</del>");
                            ...
                            socketToTargetClient.write(rawBufferFromProxyToClient)
                                    .onComplete(voidAsyncResult -> {
                                        if (voidAsyncResult.failed()) {
                                            ...
                                            NetSocket clientRemovedToClose = this.clientSocketManager.getClientSocketMap().remove(clientID);
                                            if (clientRemovedToClose != null) clientRemovedToClose.close();
                                        } else {
    ...
    public void stop() throws Exception {
        getLogger().info("解除部署 执行stop方法");
        // 关闭通信
        if (socket != null) {
            socket.close()
                    .onComplete(asyncResult -> {
                        if (asyncResult.failed()) {
                            getLogger().exception("解除部署 关闭通信 出现问题", asyncResult.cause());
                        } else {
                            getLogger().info("解除部署 关闭通信");
                        }
                    });
        }
    }
