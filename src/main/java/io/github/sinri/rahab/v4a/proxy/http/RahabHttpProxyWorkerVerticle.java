...
class RahabHttpProxyWorkerVerticle extends KeelVerticle {
    ...
    private void proxyServerConnectHandler() {
        ...
        proxySocket
                .handler(bufferFromClient -> {
                    ...
                    workerLogger.text(KeelLogLevel.DEBUG, bufferFromClient.toString(), System.lineSeparator());
                        ...
                        freedomClient.connect(port, host, netSocketAsyncResult -> {
                            ...
                            freedomSocket
                                    .handler(bufferReadFromActualServer -> {
                                        ...
                                        workerLogger.text(KeelLogLevel.DEBUG, bufferReadFromActualServer.toString(), System.lineSeparator());
    ...
    public void stop() throws Exception {
        ...
        if (this.proxySocket != null) {
            this.proxySocket.close(asyncResult -> {
                if (asyncResult.failed()) {
                    getLogger().exception("解除部署 关闭代理通讯 出错", asyncResult.cause());
                } else {
                    getLogger().info("解除部署 关闭代理通讯");
                }
            });
        }
    }
