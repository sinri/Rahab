package io.github.sinri.rahab.v4a.proxy.socks5;
...
class RahabSocks5ProxyWorkerVerticle extends KeelVerticle {
    ...
    public void start() throws Exception {
        KeelLogger socketLogger = Keel.standaloneLogger("Socks5ServerSocket")
                ...
                .handler(bufferFromClient -> {
                    ...
                    switch (protocolStepEnum.get()) {
                        case STEP_1_CONFIRM_METHOD:
                            socketLogger.text(KeelLogLevel.DEBUG, Keel.stringHelper().bufferToHexMatrix(bufferFromClient, 20), System.lineSeparator());
                            ...
                            if (step_1_ver != 0x05) {
                                ...
                                getLogger().error("客户端发来的第一步交信有问题准备解除部署: VERSION IS NOT 5");
                                undeployMe();
                                return;
                            }
                                            ...
                                            .compose(socketToActualServer -> {
                                                socketToActualServer
                                                        .handler(bufferFromActualServer -> {
                                                            ...
                                                            socketLogger.text(KeelLogLevel.DEBUG, Keel.stringHelper().bufferToHexMatrix(bufferFromActualServer, 20), System.lineSeparator());
                                                            socketLogger.text(KeelLogLevel.DEBUG, bufferFromActualServer.toString(), System.lineSeparator());
                        ...
                        case STEP_4_TRANSFER:
                            ...
                            socketLogger.text(KeelLogLevel.DEBUG, Keel.stringHelper().bufferToHexMatrix(bufferFromClient, 20), System.lineSeparator());
                            socketLogger.text(KeelLogLevel.DEBUG, bufferFromClient.toString(), System.lineSeparator());
    ...
    public void stop() throws Exception {
        ...
        if (this.socketFromClient != null) {
            this.socketFromClient.close(asyncResult -> {
                if (asyncResult.failed()) {
                    getLogger().exception("解除部署 关闭与客户端的通讯 出错", asyncResult.cause());
                } else {
                    getLogger().info("解除部署 关闭与客户端的通讯");
                }
            });
        }
    }
    ...
    private void respondInStep3(Byte repByte, Byte addressType, byte[] address, short port, NetSocket socketFromClient, KeelLogger socketLogger) {
        ...
        socketFromClient.write(bufferToRespond)
                .onFailure(throwable -> {
                    ...
                    socketLogger.text(KeelLogLevel.DEBUG, Keel.stringHelper().bufferToHexMatrix(bufferToRespond, 20), System.lineSeparator());
                })
                .onSuccess(v -> {
