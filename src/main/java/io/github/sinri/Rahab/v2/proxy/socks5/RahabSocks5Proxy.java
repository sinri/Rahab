...
public class RahabSocks5Proxy {
     ...
     * SOCKS5 代理服务器
     */
    protected NetServer socks5Server;
    /**
     * 连接目标服务器的客户端
     */
    ...
    public RahabSocks5Proxy(Set<RahabSocks5AuthMethod> supportedAuthMethodSet) {
    ...
    public Future<NetServer> listen(int port) {
        return this.socks5Server
                .connectHandler(socketFromClient -> {
                    String clientID = UUID.randomUUID().toString().replace("-", "");
                    KeelLogger socketLogger = Keel.standaloneLogger("SocksServerSocket").setCategoryPrefix(clientID);

                    socketLogger.notice("新通讯 已被建立 自 客户端 " + socketFromClient.remoteAddress().toString());

                    AtomicReference<ProtocolStepEnum> protocolStepEnum = new AtomicReference<>(ProtocolStepEnum.STEP_1_CONFIRM_METHOD);
...
//                                                        });
                ...
                .exceptionHandler(throwable -> {
                    Keel.logger("RahabSocks5Proxy").exception("RahabSocks5Proxy Error", throwable);
                })
                .listen(port);
    }
